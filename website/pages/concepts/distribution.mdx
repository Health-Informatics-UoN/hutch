...
A Distribution Task is a standardised request asking for summary statistics of the OMOP dataset, they run nightly on a dataset. The task includes metadata such as the collection ID, and task ID, along with the type (`analysis`) to be run. 
