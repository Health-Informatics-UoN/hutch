...
must be enabled with `OMOP_LOCATION_ENABLED` (see [Configuration](/bunny/config#database)), and
requires an OMOP CDM v5.4 database — see [Requirements](/bunny/deployment/requirements#omop-cdm).
...
must be enabled with `OMOP_DEATH_ENABLED` (see [Configuration](/bunny/config#database)).
