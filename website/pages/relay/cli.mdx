...
<Tabs.Tab>
  <Tabs items={["Windows", "Linux / macOS"]}>
    <Tabs.Tab>```bash Hutch.Relay.exe --help ```</Tabs.Tab>
    <Tabs.Tab>```bash ./Hutch.Relay --help ```</Tabs.Tab>
  </Tabs>
</Tabs.Tab>
  ...
  <Tabs.Tab>
    ...
    <Callout>
      Change Directory (`cd`) next to `Hutch.Relay.csproj`, or specify the project using `--project path/to/Hutch.Relay.csproj`.
    </Callout>
    ...
    For example, to use development appsettings and any local user secrets:

    ```bash
    dotnet run --environment Development -- --help
    ```
