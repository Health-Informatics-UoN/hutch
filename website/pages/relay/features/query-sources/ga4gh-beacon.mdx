...
<Callout>
Note that Relay only supports `boolean` or `count` granularity for individuals queries.
...
- Relay also supports an [Upstream Task API](upstream-task-api) as a query source
