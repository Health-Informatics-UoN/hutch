...
- Each image is published to the GitHub container registry. 
...
- Bunny should be secured with network policies to restrict outbound requests to RQUEST/Relay API.
- Bunny makes only outgoing requests to RQUEST/Relay API over HTTPS (port 443).
...
- Bunny requests to RQUEST/Relay are secured using Basic Auth, consistent with the API specification.
...
- Bunny can be configured to log externally requests, paylods, queries, SQL executed, results, and errors for audit purposes.
