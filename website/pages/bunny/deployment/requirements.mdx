...
| Location   | location_id, location_source_value, country_concept_id†, latitude†, longitude† | vX.Y onwards |

Location is joined via `person.location_id` → `location.location_id`.

_*Not included in the HDR Gateway minimum data model._

_†`country_concept_id`, `latitude`, and `longitude` were added to the Location table in OMOP CDM v5.4 — a v5.3 database only supports filtering on `location_source_value`._
