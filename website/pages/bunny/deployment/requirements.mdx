...
| Database Access     | OMOP CDM PostgreSQL / SQL Server / Snowflake / DuckDB       |
...
| DuckDB        | 1.x                            |
| Snowflake     | Tested 10.1.1 Standard Edition |

## OMOP CDM

Bunny requires an [OMOP CDM](https://ohdsi.github.io/CommonDataModel/background.html) database to query for data.

Bunny supports OMOP CDM 5.3 and 5.4, as the [differences](https://ohdsi.github.io/CommonDataModel/cdm54Changes.html) between the two are not relevant, with the exception of the Location table's `latitude` and `longitude` fields†.
...
Optional tables set by configuration:

| OMOP Table | OMOP Field Names                                           | Supported from |
|------------|------------------------------------------------------------|----------------|
| Specimen   | specimen_id, person_id, specimen_concept_id, specimen_date | v1.7 onwards   |
| Location   | location_id, latitude†, longitude†                         | v1.8 onwards |
| Death      | person_id, death_date, cause_concept_id‡                   | v1.9 onwards |

Location is joined via `person.location_id` → `location.location_id`.

_*Not included in the HDR Gateway minimum data model._

_†`latitude` and `longitude` were added to the Location table in OMOP CDM v5.4 — a v5.3 database cannot use the geo-radius filter._

_‡`cause_concept_id` is nullable — only required to allow filtering by specific causes of death, without this, Bunny is still able to query on any death record._
...
| Procedure Occurrence   | idx_procedure_concept_id_1         |
