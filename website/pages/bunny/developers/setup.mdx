...
- If you are connecting to RQUEST, read the [guidance on configuration](/bunny/how-to/connect_rquest).
...
You will need to install its dependencies by running
```bash copy
uv sync
```
...
To run Bunny without Relay a upstream API, the command-line interface can be used.
This needs a file with the right JSON schema to run (example below).

To run a query from the CLI, from root run

```bash copy
uv run bunny --body tests/queries/availability.json
```

This should write the result for your query to `output.json`
