...
```bash copy
wget https://github.com/Health-Informatics-UoN/hutch/blob/main/samples/Bunny/resolve-standalone.compose.yml
```

### Environment configuration

Modify the configuration of the newly downloaded `resolve-standalone.compose.yml` file as described in [Configuring Resolve](/resolve/config)
...
At the directory containing the `resolve-standalone.compose.yml` file, run the command to activate Resolve:

```bash copy
docker compose -f resolve-standalone.compose.yml up
```
