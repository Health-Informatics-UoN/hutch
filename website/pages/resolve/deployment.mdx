...
Modify the configuration of the newly downloaded `compose.resolve.yml` file as described in [Configuring Resolve](/resolve/config) {/* This information is in the config description, or if it isn't it should be */}

You will need to [configure the database connection](/resolve/config#database), and your [task API connection](/resolve/config#task)
